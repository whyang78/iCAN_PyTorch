...
# 80
COCO_NAMES = ['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic_light',
...
def get_predection(img, model, threshold):
...
def object_detection(frame, model, detection, threshold=0.5):
    frame=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
    pred_bboxes, pred_labels, pred_scores = get_predection(frame, model, threshold)
...
def im_detect(net, image_id, frame, detection_result, prior_mask, action_dict_inv, object_thres, human_thres,
              prior_flag, detection, device):
                ...
                if np.max(object[5]) > object_thres and not np.all(human[2] == object[2]):
                    ...
                    with torch.no_grad():
                        if args.model == 'iCAN_ResNet50_VCOCO':
                            h_score, o_score, sp_score, a, b = net(image.to(device), sp_new.to(device),
                                                                   h_box_new.to(device),
                        ...
                        if args.model == 'iCAN_ResNet50_VCOCO_Early':
                            h_score, all_score, atten_h, atten_o = net(image.to(device), sp_new.to(device),
                                                                       h_box_new.to(device),
                                                                       o_box_new.to(device), num_pos)
                    ...
                    temp_score_obj = np.concatenate((object[2].reshape(1, 4), score.reshape(1, 29) * np.max(object[5])),
                                                    axis=1)
...
def im_detect_hico(net, image_id, frame, detection_result, object_thres, human_thres, detection, device):
    all_info = []
    image = process_image(frame)
...
def get_prediction(net, detection_result, frame, prior_mask, action_dict_inv, model, object_thres, human_thres,
                   prior_flag, device):
    net.eval()
    image_id = 0
    if model == 'iCAN_ResNet50_VCOCO':
        print(1)
        detection = []
        im_detect(net, image_id, frame, detection_result, prior_mask, action_dict_inv, object_thres, human_thres,
                  prior_flag, detection, device)
    elif model == 'iCAN_ResNet50_HICO':
        print(2)
        detection = {}
        im_detect_hico(net, image_id, frame, detection_result, object_thres, human_thres, detection, device)
    return detection
...
def create_text(img, ho_dic,text, shape, count, H_box,O_box):
    ...
    cv2.putText(
        img, text, (x1 + 20, y1 + 30+(count * 40)), cv2.FONT_HERSHEY_COMPLEX_SMALL, font_size,tuple(map(int,cc[ho_dic[tuple(O_box)]])) , 2,
        cv2.LINE_AA
    )
...
def create_bbox(img, ho_dic,box, count):
    ...
    cv2.line(img, (x2, y2), (x2, y1), color, 2)
    cv2.line(img, (x2, y1), (x1, y1),color, 2)
...
def print_image_hico(Detection, im_data,img_id):
    ...
    for ele in Detection[img_id]:
        ...
        if np.any(score > 0.4):
            ...
            action_ind = np.where(score > 0.4)[0]
...
if __name__ == '__main__':
    ...
    video_path="./demo_videos/drink.mp4"
    output_path='./demo_videos/drink_o.mp4'
    ...
    for i in range(400+1):
        ...
        obj_detection={}
        object_detection(frame,model,obj_detection)

        hoi_detection=get_prediction(net, obj_detection, frame, prior_mask, action_dict_inv, args.model,
                       args.object_thres, args.human_thres, args.prior_flag, device)

        if args.model == 'iCAN_ResNet50_VCOCO':
            frame=print_image_vcoco(hoi_detection,frame)
        if args.model == 'iCAN_ResNet50_HICO':
            frame=print_image_hico(hoi_detection,frame,0)
