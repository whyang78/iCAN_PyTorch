...
def object_detection(image_path,detection,threshold=0.5):
    ...
    image_name=int(image_path.split(os.sep)[-1].split('.')[0])
    temp=[]
...
if __name__ == '__main__':
    img_folder='./demo_images'
    output_file='./detection_result/detection.pkl'
    ...
    for p in glob.iglob(img_folder+'/*.jpg'):
        image_name = int(p.split(os.sep)[-1].split('.')[0])
