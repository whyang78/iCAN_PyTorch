...
class HICO_model(nn.Module):
    ...
    def forward(self, image, spatial, h_boxes, o_boxes, num_pos):
        ...
        h_inst, atten_h, h_all = self.human_stream(head, h_boxes, self.head_phi, self.head_g, self.conv,
                                                   self.identify_block_phi, self.identify_block_g)
        h_score = self.fc_h(h_all)
        _, atten_o, o_all = self.object_stream(head, o_boxes[:num_pos, :], self.head_phi, self.head_g, self.conv,
                                               self.identify_block_phi, self.identify_block_g)
