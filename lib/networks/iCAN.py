...
class iCAN(nn.Module):
    ...
    def forward(self, base_feature, rois,head_phi_f, head_g_f, conv_f, identify_block_phi,identify_block_g,output_size=(7,7), spatial_scale=1.0/16.0, num_pos=None):
        '''
        ...
        if num_pos==None:
            ...
            fc_1 = self.fc(fc_inst[:num_pos,:])
        fc_1=fc_1.reshape(fc_1.size(0),fc_1.size(1),1,1)

        head_phi = identify_block_phi(base_feature)
        head_phi=head_phi_f(head_phi)
        head_g = identify_block_g(base_feature)
        head_g=head_g_f(head_g)
        ...
        if num_pos==None:
            ...
            stream = torch.cat((fc_inst[:num_pos, :], fc_context), dim=1)
    ...
    def roi_align(self,base_feature,rois,output_size,spatial_scale):
        ...
        roi_align = ops.RoIAlign(output_size=output_size, spatial_scale=spatial_scale,sampling_ratio=2)
